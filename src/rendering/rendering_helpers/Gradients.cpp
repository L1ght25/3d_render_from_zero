...
rendering::Gradients::Gradients(const Vertex& min_y_v, const Vertex& mid_y_v, const Vertex& max_y_v)
    ...
    z_step_(0, 0) = ((one_over_z_(1, 0) - one_over_z_(2, 0)) * (min_y_v.GetY() - max_y_v.GetY()) -
                     (one_over_z_(0, 0) - one_over_z_(2, 0)) * (mid_y_v.GetY() - max_y_v.GetY())) *
                    (1.f / dX);

    z_step_(1, 0) = ((one_over_z_(1, 0) - one_over_z_(2, 0)) * (min_y_v.GetX() - max_y_v.GetX()) -
                     (one_over_z_(0, 0) - one_over_z_(2, 0)) * (mid_y_v.GetX() - max_y_v.GetX())) *
                    (1.f / dY);
    ...
    depth_step_(0, 0) =
        ((depth_(1, 0) - depth_(2, 0)) * (min_y_v.GetY() - max_y_v.GetY()) - (depth_(0, 0) - depth_(2, 0)) * (mid_y_v.GetY() - max_y_v.GetY())) *
        (1.f / dX);
    depth_step_(1, 0) =
        ((depth_(1, 0) - depth_(2, 0)) * (min_y_v.GetX() - max_y_v.GetX()) - (depth_(0, 0) - depth_(2, 0)) * (mid_y_v.GetX() - max_y_v.GetX())) *
        (1.f / dY);
}
...
geometry::Vector4d rendering::Gradients::calc_step(Vector4d* vec, const Vertex& min_y_v, const Vertex& mid_y_v, const Vertex& max_y_v, double diff,
                                                   bool is_x) {
